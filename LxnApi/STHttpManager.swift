...
private class STSingletonKey {
    static let _shareManager = STSingletonKey()
    class func shareManager() -> STSingletonKey {
        return _shareManager
    }
    var fetchRequestKeys = [String]()
}

public class STHttpManager<T> {
//    STSingletonKey.shareManager()
    // 用来处理只请求一次的栅栏队列
...
extension STHttpManager where T: Codable {
    ...
    func requestModel<R: TargetType & MoyaAddable>(
        ...
        completion: @escaping ((STModelResponse<T>?) -> Void),
        error: @escaping (STHttpError) -> Void )
        -> Cancellable? {
    ...
    func requestListModel<R: TargetType & MoyaAddable>(
        _ type: R,
        test: Bool = false,
        completion: @escaping ((STListResponse<T>?) -> Void),
        error: @escaping (STHttpError) -> Void )
        -> Cancellable? {
...
extension STHttpManager {
    ...
    private func request<R: TargetType & MoyaAddable>(
        ...
        test: Bool = false,
        progressBlock: ((Double) -> Void)? = nil,
        modelComletion: ((STModelResponse<T>?) -> Void)? = nil,
        modelListComletion: ((STListResponse<T>?) -> Void )? = nil,
        error: @escaping (STHttpError) -> Void )
        -> Cancellable? {
        // 同一请求正在请求直接返回
        ...
        let cancellable = provider.request(type, callbackQueue: DispatchQueue.global(), progress: { (progress) in
            ...
            let errorblock = { (e: STHttpError) in
                DispatchQueue.main.async {
                    error(e)
                }
            }
            ...
            case .failure:
                errorblock(STHttpError.exception(message: "未连接到服务器"))
            }
        }
        return cancellable
    }
    ...
    private func handleSuccessResponse<R: TargetType & MoyaAddable>(
        ...
        case .uploadMultipart, .requestParameters:
            ...
            } catch let STHttpError.serverResponse(message, code) {
                error(STHttpError.serverResponse(message: message, code: code))
            } catch let STHttpError.loginStateIsexpired(message) {
                // 登录状态变化清楚登录缓存信息
                error(STHttpError.loginStateIsexpired(message: message))
            } catch {
                #if Debug
                fatalError("未知错误")
                #endif
    ...
    // 处理数据
    private func handleResponseData<R: TargetType & MoyaAddable>(_ isList: Bool, type: R, data: Data) throws -> (STModelResponse<T>?, STListResponse<T>?) {
        guard let jsonAny = try? JSONSerialization.jsonObject(with: data, options: []) else {
            throw STHttpError.jsonSerializationFailed(message: "JSON解析失败")
        }
        
        if isList {
            let listResponse: STListResponse<T>? = STListResponse(data: jsonAny)
            guard let temp = listResponse else {
                throw STHttpError.jsonToDictionaryFailed(message: "JSON转字典失败")
            }
            ...
            let response: STModelResponse<T>? = STModelResponse(data: jsonAny)
    ...
    private func handleCode(responseCode: Int, message: String?) throws {
        ...
        case ResponseCode.forceLogoutError:
            throw STHttpError.loginStateIsexpired(message: message)
        default:
            throw STHttpError.serverResponse(message: message, code: responseCode)
        }
    }
    ...
    private func cacheData<R: TargetType & MoyaAddable>(
        ...
        modelComletion: ((STModelResponse<T>?) -> Void)? = nil,
        modelListComletion: ( (STListResponse<T>?) -> Void )? = nil,
        model: (STModelResponse<T>?, STListResponse<T>?)) {
